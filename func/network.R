...
#' @param df_edges data.frame. "from", "to", "weight".
#' @param dict.combine data.frame. "id", "label", "term", "semantic_type", "group2", "group", "type", "category"
#' @param attrs list. attr_nodes_type attr_nodes_center attr_edges attr_legend_edge(if directed)
#' @param colors_group data.frame. "group", "color.background".
#' @param shapes data.frame. "type", "shape".
...
dataNetwork <- function(center_nodes, df_edges, dict.combine, attrs, 
  ...
  df_edges$title <- paste(df_edges$from, "<b> &rarr; </b>", df_edges$to, ": ", round(df_edges$weight, 3))
  ...
  df_edges <- df_edges %>%
    ...
    dplyr::summarise(from = gsub("\\|.+", "", .data$ends[1], perl = TRUE),
                     ...
                     length = mean(abs(.data$weight)^(-1.1)*10),
                     title = .data$title[1])
  ...
  df_nodes <- left_join(df_nodes, shapes, by = "type")
  ...
  if(length(unique(legends$shape)) > 1){
    for (s in unique(shapes$shape)){
      if(s %in% legends$shape){
        legends <- dplyr::add_row(
          legends,
          data.frame("label" = shapes$type[match(s, shapes$shape)],
                     "shape" = "box",
                     "color" = "rgba(0,0,0,0)",
                     "size" = 10,
                     "font.size" = 20,
